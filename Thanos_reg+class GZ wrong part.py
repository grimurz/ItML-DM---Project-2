...
CV2 = model_selection.KFold(n_splits=K, shuffle=True, random_state = 43)

# Initialize variables
Error_par_lin = np.empty(K)
mse_par_ridge = np.zeros([K,len(lambdas)])
mse_par_lasso = np.zeros([K,len(lambdas)])
mse_par_elastic = np.zeros([K,len(lambdas)])

Error_test_lin = np.empty(K)
...
for par_index, test_index in CV.split(Xr):
    print('Computing outer linear CV fold: {0}/{1}'.format(q+1,K))

    # extract par and test set for outer CV fold
    Xr_par_KFold_lin, yr_par_KFold_lin = Xr[par_index,:], yr[par_index]
    Xr_test_KFold_lin, yr_test_KFold_lin = Xr[test_index,:], yr[test_index]

    # Initialize variables
    Error_train_lin = np.empty(K)
    mse_train_ridge = np.zeros([K,len(lambdas)])
    mse_train_lasso = np.zeros([K,len(lambdas)])
    mse_train_elastic = np.zeros([K,len(lambdas)])

    Error_val_lin = np.empty(K)
    mse_val_ridge = np.zeros([K,len(lambdas)])
    mse_val_lasso = np.zeros([K,len(lambdas)])
    mse_val_elastic  = np.zeros([K,len(lambdas)])
 
    k=0
    ratio=0
    
    for train_index, val_index in CV.split(Xr_par_KFold_lin):

        # extract train and val set for inner CV fold
        Xr_train_KFold_lin, yr_train_KFold_lin = Xr[train_index,:], yr[train_index]
        Xr_val_KFold_lin, yr_val_KFold_lin = Xr[val_index,:], yr[val_index]
        ...
        for i, lam in enumerate(lambdas):
            ...
            y_val_pred_ridge = ridge_reg.predict(Xr_val_KFold_lin)
    
            # Calculate error
            mse_train_ridge[k,i] = np.mean((y_train_pred_ridge-yr_train_KFold_lin)**2)
            mse_val_ridge[k,i] = np.mean((y_val_pred_ridge-yr_val_KFold_lin)**2)
        ...
        for h, alph in enumerate(lambdas):
            ...
            # Fit ridge regression model
            lasso_reg = make_pipeline(PolynomialFeatures(2), Lasso(alpha=alph, tol=0.05))
            lasso_reg.fit(Xr_train_KFold_lin, yr_train_KFold_lin)
            # Compute model output:
            y_train_pred_lasso = lasso_reg.predict(Xr_train_KFold_lin)
            y_val_pred_lasso = lasso_reg.predict(Xr_val_KFold_lin)
    
            # Calculate error
            mse_train_lasso[k,h] = np.mean((y_train_pred_lasso-yr_train_KFold_lin)**2)
            mse_val_lasso[k,h] = np.mean((y_val_pred_lasso-yr_val_KFold_lin)**2)
        ...
        for l, alp in enumerate(lambdas):
            ...
            # Fit ridge regression model
            elastic_reg = make_pipeline(PolynomialFeatures(2), ElasticNet(alpha=alp, l1_ratio=ratio, tol=0.5))
            elastic_reg.fit(Xr_train_KFold_lin, yr_train_KFold_lin)
            # Compute model output:
            y_train_pred_elastic = elastic_reg.predict(Xr_train_KFold_lin)
            y_val_pred_elastic = elastic_reg.predict(Xr_val_KFold_lin)
    
            # Calculate error
            mse_train_elastic[k,l] = np.mean((y_train_pred_elastic-yr_train_KFold_lin)**2)
            mse_val_elastic[k,l] = np.mean((y_val_pred_elastic-yr_val_KFold_lin)**2)
        ...
        lin_test_pred = lin_reg.predict(Xr_val_KFold_lin)
        lin_train_pred = lin_reg.predict(Xr_train_KFold_lin)
        
        misclass_rate_val = np.square(yr_val_KFold_lin-lin_test_pred).sum()/yr_val_KFold_lin.shape[0]
        misclass_rate_train = np.square(yr_train_KFold_lin-lin_train_pred).sum()/yr_train_KFold_lin.shape[0]
        Error_val_lin[k], Error_train_lin[k] = misclass_rate_val, misclass_rate_train
...
# plt.semilogx(lambdas, np.sqrt(Error_test_lin.mean()*np.ones(50)),label='Linear test error')
...
# plt.legend(['Ridge train error','Ridge test error','Linear test error','Lasso train error','Lasso test error','ElasticNet train error','ElasticNet test error','Ridge minimum'],loc='upper right')
# plt.ylim([5, 15])
# plt.grid()
# plt.show()  
...
# del k, K, i, lam, ridge_reg, train_index, test_index, par_index
